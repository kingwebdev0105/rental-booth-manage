<body id="user_page">
	<div class="flex-row" style="padding: 0 10px; margin: 10px 0;">
			...
			<div class="flex-row" style="justify-content: space-evenly;">
				...
				<div class="flex-col">
					...
					<div class="control-panel" style="visibility: hidden;">
						<button class="entrance-room-btn"><span>入&nbsp&nbsp&nbsp室</span></button>
						<button class="leave-room-btn"><span>退&nbsp&nbsp&nbsp室</span></button>
					</div>
				</div>
			</div>
	...
	<div id="toolbar" style="padding: 10px 50px; background-color: powderblue;">
		<a style="font-size: 1.2rem;" class="margin-right-15em" href="<?php echo base_url('/') ?>">管理者画面へ</a>
		<button id="full-screen-btn"><span>全画面表示</span></button>
	</div>
	...
	<script>
		...
		conn.onmessage = function (e) {
			...
			if (data.message) {
				updateBooth(data.message);
			}
		};

		function updateBooth(recvData) {

			var elem = $('[data-id="' + recvData[0] + '"]');

			switch(parseInt(recvData[1])) {
				case 0:
					if(elem.hasClass('room-entranced')) {
						elem.removeClass("room-entranced");
					} else if(elem.hasClass('room-paused')){
						elem.removeClass("room-paused");
					}
					
					showRemainSeats(recvData);
					break;
				case 1:
					elem.addClass("room-entranced");
					showRemainSeats(recvData);
					break;
				case 2:
					elem.addClass("room-paused");
					showRemainSeats(recvData);
					break;
				case 3:
		...
		function showRemainSeats(data) {
			$('#perabo').text(data[2]);
			$('#perabo_pair').text(data[3]);
			$('#executive').text(data[4]);
			$('#premium').text(data[5]);
		}
		...
		$(document).on('fullscreenchange webkitfullscreenchange mozfullscreenchange MSFullscreenChange', function() {
			if(IsFullScreenCurrently()) {
				$("#full-screen-btn span").text('全画面取消');
				$('#toolbar').hide();
			}
			else {
				$("#full-screen-btn span").text('全画面表示');
				$('#toolbar').show();
			}
		});
